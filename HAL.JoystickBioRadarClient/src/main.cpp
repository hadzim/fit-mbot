...
HAL::API::BioRadar::DBus::Client::Ptr client;
...
void onJoystick(Joystick::Coord & c) {
	try {
			...
			double speed = c.y;
			std::cout << tst.elapsed() / 1000 << "ms" << " speed: " << c.y << std::endl;
			tst.update();
			client->BioRadar().GoRelBase(speed * 100);
...
int main(int argc, char** argv) {
	try {
		client = new HAL::API::BioRadar::DBus::Client();
		{
			SDLLoop loop;
			{
				Joystick j(0);
