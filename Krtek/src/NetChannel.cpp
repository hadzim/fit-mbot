...
namespace TBS {
	...
	TBS::Robo::RoboCan::CanMessage messageFromFrame(CanFrame frame) {
	...
	CanFrame frameFromMessage(TBS::Robo::RoboCan::CanMessage msg) {
		CanFrame sendframe;
	...
	void NetChannel::sendCanMessage(const TBS::Robo::RoboCan::CanMessage & canMessage) {
	...
	void NetChannel::run() {
		try {
			...
			while (!stopThread) {
				...
				for (CanMessages::iterator msg = toSend.begin(); msg != toSend.end(); msg++) {
					if (stopThread)
						break;
				...
				for (CanFrame::Queue::iterator i = received.begin(); i != received.end(); i++) {
