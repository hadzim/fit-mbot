...
SerialWrapper::VectorBuffer SerialWrapper::receive(int waitTime, int length) {
	...
	while (slept < waitTime) {
		...
		if (finalBuffer.size() >= length) break;
...
SerialWrapper::VectorBuffer SerialWrapper::receive(
		SerialWrapper::ListBuffer charSequence, int waitTime) {
	int slept = 0;
	...
	while (slept < waitTime) {
		...
		finalBuffer.insert(finalBuffer.end(), tmpBuffer.begin(),
				tmpBuffer.end());

		//check sequence
		SerialWrapper::ListBuffer charSequenceCpy = charSequence;
		for (SerialWrapper::VectorBuffer::iterator i = finalBuffer.begin();
				i != finalBuffer.end(); i++) {
