...
#ifndef INNERMODULE_H_
#define INNERMODULE_H_
#include <string>
...
namespace TBS {
	namespace Robo {
		namespace RoboCan {
				...
				class ICanModule : public TBS::Robo::IModule {
					public:
						typedef Poco::SharedPtr<IModule> Ptr;
						typedef IModule * RawPtr;
						...
						enum Commands {
							CMD_GET_STATUS = 0, CMD_GET_DATA = 1, CMD_GET_ERROR = 2, CMD_START = 3, CMD_HALT = 4, CMD_RESTART = 5, CMD_STOP = 6,
						};

						ICanModule(Name name, ICanNode::RawPtr node, int numberWithinNode);
						virtual ~ICanModule();
						/*
						 int getNumberWithinNode() const;
						 void setNumberWithinNode(int n);
						 */
						CanID getCanID() const;
						CanID getNodeID() const;

						const std::string & getName() const;

					private:
						std::string name;
						ICanNode::RawPtr node;
