...
#ifndef safe_Timer_INCLUDED
#define safe_Timer_INCLUDED
...
namespace TBS {
		...
		class TBS_API SafeTimer: protected Poco::Runnable {
			...
			private:
				...
				SafeTimer(const SafeTimer&);
				SafeTimer& operator =(const SafeTimer&);
		};

		class TBS_API AbstractTimerCallback
		...
		/// the TimerCallback template.
		{
			public:
				...
				virtual void invoke(SafeTimer& timer) const = 0;
		...
		{
			public:
				typedef void (C::*Callback)(SafeTimer&);

				TimerCallback(C& object, Callback method)
						: _pObject(&object), _method(method) {
				}

				TimerCallback(const TimerCallback& callback)
						: AbstractTimerCallback(callback), _pObject(callback._pObject), _method(callback._method) {
				}
				...
				void invoke(SafeTimer& timer) const {
