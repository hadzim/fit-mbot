...
#define SYSTEC_MODULE_VERSION   "0.8.0"
...
#define REQ_FW_MAJOR                       5
#define REQ_FW_MINOR                       2
#define REQ_FW_RELEASE_1103                60
#define REQ_FW_RELEASE_1104                52
#define REQ_FW_RELEASE_1105                35
#define REQ_FW_RELEASE_1121                9
#define REQ_FW_RELEASE_1122                7
#define REQ_FW_RELEASE_1144                52
#define REQ_FW_RELEASE_1145                35
...
struct systec_pid_chan {
        ...
        u32 fw_address;          /* firmware start address            */
        u32 buffer_size;         /* update buffer size                */
};

/* define product id and required firmware version */
static struct systec_pid_chan systec_pid_chan_table[] = {
        {0x1103, FW_VERSION(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1103), 0x00006000, 0x00006000}, /* 8 or 16 channel device generation 3 */
        {0x1104, FW_VERSION(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1104), 0x00006000, 0x00006000}, /* single channel device generation 3 */
        {0x1105, FW_VERSION(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1105), 0x00006000, 0x00006000}, /* dual channel device generation 3 */
        {0x1121, FW_VERSION(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1121), 0x08008000, 0x00008000}, /* dual channel device generation 4 */
        {0x1122, FW_VERSION(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1122), 0x08008000, 0x00008000}, /* single channel device generation 4 */
        {0x1144, FW_VERSION(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1144), 0x00006000, 0x00006000}, /* CANUSB4 KSPS-0809 */
        {0x1145, FW_VERSION(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1145), 0x00006000, 0x00006000}, /* 3204013 */
        {0, 0}
};
                            ...
                            int pid_index)
{
        ...
        current_version = get_unaligned_le32(&fw_record->data[0]);
        systec_dev_dbg(SYSTEC_DEBUG_DRIVER, dev, "   found firmware header version: %d.%d r%d",
        ...
        current_version = get_unaligned_le32(&fw_record->data[4]);
        ...
        product_id = get_unaligned_le16(&fw_record->data[8]);
        ...
        if (le32_to_cpu(update_buf[0]) == 0) {
                ...
                update_buf[0] = cpu_to_le32(systec_pid_chan_table[pid_index].fw_address);
                update_buf[1] = cpu_to_le32(systec_pid_chan_table[pid_index].buffer_size);
                update_buf[2] = 0;
                            ...
                            const struct usb_device_id *id)
{
        ...
        pid_index = 0;
        while (systec_pid_chan_table[pid_index].pid != 0
                && id->idProduct != systec_pid_chan_table[pid_index].pid) {

                pid_index++;
        }

        if (systec_pid_chan_table[pid_index].pid == 0) {
                /* should never happen */
        ...
        required_fw_version = systec_pid_chan_table[pid_index].required_fw_version;
        ...
        dev_info(dev, "device firmware version: %d.%d r%d",
                ...
                GETPC_RELEASE_VER(current_version));
        systec_dev_dbg(SYSTEC_DEBUG_DRIVER, dev, "version required by driver: %d.%d r%d",
        ...
        if (request_firmware(&fw, fw_name, dev) == 0) {
                ...
                ret_stat = systec_can_update_firmware(intf, fw, id->idProduct, required_fw_version, pid_index);
                release_firmware(fw);
                if (ret_stat != 0)
                {
                        return ret_stat;
...
MODULE_FIRMWARE(FIRMWARE_FILE(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1121, 1121));
MODULE_FIRMWARE(FIRMWARE_FILE(REQ_FW_MAJOR, REQ_FW_MINOR, REQ_FW_RELEASE_1122, 1122));
