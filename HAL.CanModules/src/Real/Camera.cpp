...
namespace MBot {
	...
	Camera::Camera(TBS::NotificationWorker::Ptr nw, TBS::Robo::RoboCan::IChannel::Ptr canChannel) :
			...
			speed(20), maxRelDurationTimeInMs(50), t("CameraWorker"), finished(false), enabled(false) {
	...
	static void internalGoMin(TBS::Task::OneActiveTaskExectution & execution, MBot::CameraMotorModule & module) {
		execution.addTask(module.taskGoMin());
	}
	static void internalGoMax(TBS::Task::OneActiveTaskExectution & execution, MBot::CameraMotorModule & module) {
		execution.addTask(module.taskGoMax());
	}
	static void internalGoRel(TBS::Task::OneActiveTaskExectution & execution, MBot::CameraMotorModule & module, double speed, int maxDuration) {
		if (speed == 0) {
	...
	void Camera::GoRelEngine(const double & speed) {
		internalGoRel(motorExecution, motorModule, speed, maxRelDurationTimeInMs * 5);
	}
