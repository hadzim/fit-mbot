...
namespace MBot {
	...
	CameraMotorModule::CameraMotorModule(const std::string & name, TBS::Robo::RoboCan::ICanNode::RawPtr node, int numberWithinModule) :
	...
	TBS::Robo::RoboCan::ModuleCommandTask::Ptr CameraMotorModule::taskGoRel(Poco::Int16 speed, Poco::Int16 timeInMs) const {
		TBS::Robo::RoboCan::RoboCanMessageData data;
	...
	TBS::Robo::RoboCan::ModuleCommandTask::Ptr CameraMotorModule::taskEnable(bool enable) const {
		TBS::Robo::RoboCan::RoboCanMessageData data;
		data.setUSHORT(enable ? 1 : 0);
		return new TBS::Robo::RoboCan::ModuleCommandTask(enable ? "Enable" : "Disable" , this->getInternalModule(), this->getInternalModule().composeCommand((int) Enable, data));
	}
