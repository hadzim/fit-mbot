...
bool Dispatcher::has_something_to_dispatch() {
	...
	for (Connection::PrivatePList::iterator it = _pending_queue.begin(); it != _pending_queue.end() && !has_something; ++it) {
...
void Dispatcher::dispatch_pending() {
	while (true) {
		...
		Connection::PrivatePList pending_queue_copy;
		{
			...
			pending_queue_copy = _pending_queue;
		}
...

		{
			...
			Connection::PrivatePList::iterator from = _pending_queue.begin();
			Connection::PrivatePList::iterator to = from;
			for (int a = 0; a < copy_elem_num; a++){
				to++;
			}
...
void Dispatcher::dispatch_pending(Connection::PrivatePList &toDispatch) {
	// SEEME: dbus-glib is dispatching only one message at a time to not starve the loop/other things...

	Poco::Mutex::ScopedLock l(mutex_p_copy);
	while (toDispatch.size() > 0) {
...
void DBus::_init_threading(MutexNewFn m1, MutexFreeFn m2, MutexLockFn m3, MutexUnlockFn m4, CondVarNewFn c1, CondVarFreeFn c2, CondVarWaitFn c3,
        CondVarWaitTimeoutFn c4, CondVarWakeOneFn c5, CondVarWakeAllFn c6) {
#ifndef DBUS_HAS_RECURSIVE_MUTEX
	DBusThreadFunctions functions = { DBUS_THREAD_FUNCTIONS_MUTEX_NEW_MASK | DBUS_THREAD_FUNCTIONS_MUTEX_FREE_MASK | DBUS_THREAD_FUNCTIONS_MUTEX_LOCK_MASK
	        | DBUS_THREAD_FUNCTIONS_MUTEX_UNLOCK_MASK | DBUS_THREAD_FUNCTIONS_CONDVAR_NEW_MASK | DBUS_THREAD_FUNCTIONS_CONDVAR_FREE_MASK
	        | DBUS_THREAD_FUNCTIONS_CONDVAR_WAIT_MASK | DBUS_THREAD_FUNCTIONS_CONDVAR_WAIT_TIMEOUT_MASK | DBUS_THREAD_FUNCTIONS_CONDVAR_WAKE_ONE_MASK
	        | DBUS_THREAD_FUNCTIONS_CONDVAR_WAKE_ALL_MASK, (DBusMutexNewFunction) m1, (DBusMutexFreeFunction) m2, (DBusMutexLockFunction) m3,
	        (DBusMutexUnlockFunction) m4, (DBusCondVarNewFunction) c1, (DBusCondVarFreeFunction) c2, (DBusCondVarWaitFunction) c3,
