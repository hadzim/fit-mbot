...
namespace jsonrpc
{
    Json::Value SpecificationWriter::toJsonValue(Procedure::Map& procedures)
    {
    	Procedure::Map::iterator it;
        Json::Value result;
        Json::Value row;
        int i=0;
        for(it = procedures.begin(); it != procedures.end(); it++)
        {
            Procedure::Ptr proc = it->second;
    ...
    std::string SpecificationWriter::toString(Procedure::Map& procedures)
    {
        Json::StyledWriter wr;
        return wr.write(toJsonValue(procedures));
    }

    void SpecificationWriter::toFile(const std::string &filename, Procedure::Map& procedures)
    {
        ofstream file;
    ...
    void SpecificationWriter::procedureToJsonValue(Procedure::Ptr procedure, Json::Value &target)
    {
